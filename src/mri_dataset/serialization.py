...
def save_rendered_state(backend, state, state_dir: Path) -> Path:
    ...
    try:
        ...
        camera_height = float(backend.config.scene_value(state.scene_id, "bev_camera_height_m"))
        state.bev = {
            **mapping.metadata(),
            "files": {
            ...
            "height_depth_conversion": "invert Habitat-Sim 0.3.3 perspective-style unprojection, then orthographic linear depth",
            "height_depth_validation": backend.validate_height_rays(state, outputs["height"], seed=state.random_seed),
        }
        if outputs["bev_instance"] is not None:
